...
import { ListTheatersQuery, SearchAddressesQuery } from '@application/query/dto';
import {
...
describe('Log masking metadata', () => {
  it('민감한 application command 필드에 로그 마스킹 메타데이터를 설정한다', () => {
    expect(
      maskLoggingValue(LoginMemberCommand.of({ userId: 'member_01', password: 'password123!' })),
    ).toMatchObject({
      password: '******',
    });
    expect(
      maskLoggingValue(RefreshMemberTokenCommand.of({ refreshToken: 'refresh-token-0001' })),
    ).toMatchObject({
      refreshToken: '******',
    });
    expect(
    ...
    expect(
      maskLoggingValue(RequestPhoneVerificationCommand.of({ phoneNumber: '01012345678' })),
    ).toMatchObject({
    ...
    expect(
      maskLoggingValue(SearchAddressesQuery.of({ keyword: '서울특별시 강남구 테헤란로 427' })),
    ).toMatchObject({
      keyword: '******',
    });
    expect(
      maskLoggingValue(ListTheatersQuery.of({ latitude: 37.5005, longitude: 127.0364 })),
    ).toMatchObject({
      latitude: '******',
      longitude: '******',
    });
  ...
  it('민감한 presentation request 필드에 로그 마스킹 메타데이터를 설정한다', () => {
    expect(
      maskLoggingValue(LoginMemberRequestDto.of({ userId: 'member_01', password: 'password123!' })),
    ).toMatchObject({
      password: '******',
    });
    expect(
      maskLoggingValue(RefreshMemberTokenRequestDto.of({ refreshToken: 'refresh-token-0001' })),
    ).toMatchObject({
      refreshToken: '******',
    });
    expect(
    ...
    expect(
      maskLoggingValue(RequestPhoneVerificationRequestDto.of({ phoneNumber: '01012345678' })),
    ).toMatchObject({
      phoneNumber: '010-****-5678',
    });
    expect(
    ...
    expect(
      maskLoggingValue(SearchAddressesRequestDto.of({ keyword: '서울특별시 강남구 테헤란로 427' })),
    ).toMatchObject({
      keyword: '******',
    });
    expect(
      maskLoggingValue(ListTheatersRequestDto.of({ latitude: 37.5005, longitude: 127.0364 })),
    ).toMatchObject({
      latitude: '******',
      longitude: '******',
    });
